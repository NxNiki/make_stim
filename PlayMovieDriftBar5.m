...
GratingCycles = [6,8,6,8];
BlocksForEachCondition = 3;

CameraFPS = 30;
DataFPS = 7.5;
PostStimDur = 8;
TotalDuration = ((Duration*NumSweeps+InterBlockRest)*length(MoveDirection)*BlocksForEachCondition)/60;
SingleStimDuration = (Duration*NumSweeps+PostStimDur)*1000;
NumCameraFrames = (Duration*NumSweeps+PostStimDur)*CameraFPS;
NumDataFrames = (Duration*NumSweeps+PostStimDur)*DataFPS;
NumPostStimFrames = PostStimDur*DataFPS;
...
try
    ...
    flashdurframes = frameRate/FlashFrequency/2;
    ...
    for i_block = 1:BlocksForEachCondition
        for j = 1:length(MoveDirection);
            ...
            BarRectSplit = SplitBarRect(BarRect, GratingCycles(stimSeq(j,i_block)), screenRect);
